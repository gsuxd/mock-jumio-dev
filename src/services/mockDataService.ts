...
export const getVerificationStatusFromEmail = (email?: string): 'APPROVED_VERIFIED' | 'REJECTED_UNSUPPORTED_ID_TYPE' | 'REQUIRES_MANUAL_REVIEW' => {
  if (!email) {
...
export const generateMockDocumentData = (email?: string) => {
  ...
  return {
    ...
    documentNumber: 'DL' + Math.random().toString(36).substring(2, 11).toUpperCase(),
    address: {
...
export const generateMockVerificationResult = (email?: string, userReference?: string) => {
  const status = getVerificationStatusFromEmail(email);
  const isApproved = status === 'APPROVED_VERIFIED';
